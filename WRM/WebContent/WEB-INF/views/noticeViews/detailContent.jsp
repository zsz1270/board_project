...
<body>
	...
	<div class="contents" style="width: 800px;">
		<div class="back">
			<input id="btnback" type="button"
				onclick="location.href='/noticeViews/boardList.do'"
				value="전체목록" />
		</div>
			...
			<table id="tbMain" style="width: 100%;">
				...
				<tr>
					...
					<td style="word-break: break-all"><c:out
							value="${detailContents.get('CON_TXT')}" /></td>
				</tr>
			</table>
		</div>
		<div id="ctBottom" style="height: 25px; text-align: center;">
			<input type="button" class="btn" id="btnBefore"
				onclick="move(${detailContents.get(" CON_NO")-1})" value="&lt;&lt;" />
			<input type="button" class="btn" id="btnUpdate"
				onclick="identification(2, ${detailContents.get("CON_NO")});"value="수정" />
			<input type="button" class="btn" id="btnDelete" 
			 	onclick="identification(3, ${detailContents.get("CON_NO")});" value="삭제" /> 
	...
	<form name="content" style="width: 0px; height: 0px;">
		...
		<input type="hidden" name="con_title" /> <input type="hidden"
			name="con_txt" />
	</form>
</body>
</html>