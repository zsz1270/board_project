...
<script>
	window.onload = function(){
		...
		if (flag == true && key == 2) {
		    ...
		    document.identify.action = "/noticeViews/editContent.do";
		...
		// 삭제 본인인증 판단 후
		    ...
		    document.identify.key.value = key;
		    document.identify.con_no.value = "${boardInfo.get('con_no')}";
		    document.identify.action = "/noticeViews/delete.do";
		    document.identify.method = "POST"
		    document.identify.submit();
		    self.close();
		}
...
<body>
	<div class="contents">
		...
		<form name="beforeIdentify"
			action="/noticeViews/checkIdentify.do" method="POST">
			<!-- 본인인증 하기 전 -->
			<div id="privateMain">
				<table>
					<tr>
						<td style="background-color: #EDEDED;">ID</td>
						<td><input type="text" name="con_id" class="txtBox"
							maxlength="10" placeholder="user name"></td>
					</tr>
					<tr>
						<td style="background-color: #EDEDED;">PW</td>
						<td><input type="password" name="con_password" class="txtBox"
							maxlength="10" placeholder="＊＊＊＊"></td>
					</tr>
				</table>
			</div>
			<!-- 수정페이지에 전달하기 위해 사용 -->
			<input type="hidden" name="con_no" /> <input type="hidden"
				name="con_txt" /> <input type="hidden" name="con_title" /> <input
		...
		<form name="identify">
			<input type="hidden" name="key" /> <input type="hidden"
				name="con_no" /> <input type="hidden" name="con_title" /> <input
				type="hidden" name="con_txt" />
		</form>
		...
		<div id="privateBottom">
			...
			<c:if test="${boardInfo.get('key') eq 2}">
				<div>
					<input class="button-cancel" id="btnCancel" type="button"
						onclick="window.close();" value="Cancel" />
					<!-- 비동기 처리 수정 -->
					<input class="button" type="button" id="btnCancel" value="OK"
						onClick="submit(${boardInfo.get('con_no')});" />
				</div>
			</c:if>
			<c:if test="${boardInfo.get('key') eq 3}">
				<div>
					<input class="button-cancel" id="btnCancel" type="button"
						onclick="window.close();" value="Cancel" />
					<!-- 비동기 처리 삭제 -->
					<input class="button" id="btnCancel" type="button" value="OK"
						onClick="del(${boardInfo.get('con_no')});" />
				</div>
			</c:if>
		</div>
	</div>
