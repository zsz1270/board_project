...
<body>
	...
	<div id="contents" style="width: 800px;">
		<div id="mainHead" style="margin: 0 0 50px 0;">
			<input id="btnEdit" type="button" onclick="location.href='/board'"
				value="메인화면" />
			<h2>공지사항</h2>
		</div>
				...
				<div id="searchBar"
					...
					<form action='<c:url value="/noticeViews/boardList.do" />'
						...
						<select name="searchType">
							<option value="title"
								<c:if test="${SearchAndPagingData.get('searchType') eq 'title'}"> selected </c:if>>제목</option>
							<option value="id"
								<c:if test="${SearchAndPagingData.get('searchType') eq 'id'}"> selected </c:if>>작성자</option>
						</select> <input type="text" name="keyword" placeholder="내용을 입력해주세요."
							value="${SearchAndPagingData.get('keyword')}" /> <input
				...
				<table style="width: 100%;">
					...
					<c:if test="${boardList ne null}">
						<c:forEach items="${boardList}" var="boardList">
							<tr>
								...
								<td><a id="ctline"
									href="javascript:content(${boardList.get('CON_NO')});"><c:out
											value="${boardList.get('CON_TITLE')}" /></a></td>
					...
					<c:if test="${empty boardList}">
						<c:if test="${SearchAndPagingData.get('searchType') eq 'title'}">
							<tr>
								<td colspan="5" style="text-align: center;">제목에
									${SearchAndPagingData.get('keyword')}가 포함된 게시글이 존재하지않습니다.</td>
							</tr>
						</c:if>
						<c:if test="${SearchAndPagingData.get('searchType') eq 'id'}">
							<tr>
								<td colspan="5" style="text-align: center;">등록자명에
									${SearchAndPagingData.get('keyword')}가 포함된 게시글이 존재하지않습니다.</td>
							</tr>
						</c:if>
					</c:if>
			...
			<div id="pagebar" style="display: inline-block;" class="pagination">
				<ul class="ulPage"
					...
					<c:if test="${SearchAndPagingData.prev}">
						<li><a
							href="javascript:paging(${SearchAndPagingData.startPage - 1})">&lt;</a></li>
					</c:if>


					<c:forEach begin="${SearchAndPagingData.get('startPage')}"
						end="${SearchAndPagingData.get('endPage')}" var="idx">
						<li
							<c:out value="${SearchAndPagingData.page == idx ? 'class=active' : '' }"/>>
					...
					<c:if
						test="${SearchAndPagingData.next && SearchAndPagingData.endPage > 0}">
						<li><a
							href="javascript:paging(${SearchAndPagingData.endPage + 1})">&gt;</a></li>
					</c:if>
				</ul>
			</div>
			<div id="write_btn"
				style="float: right; display: inline-block; width: 100px; text-align: right;">
				<input type="button" id="btnEdit" onclick="key(1)" value="글쓰기" />
			</div>
		</div>
	</div>
	<form name="content" style="width: 0px; height: 0px;">
		<input type="hidden" name="con_no" /> <input type="hidden" name="key" />
	</form>
	<form name="paging">
		<!-- 상세보기 와 글쓰기 판단 후 POST 하기 위한 FORM -->
