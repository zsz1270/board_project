...
<script>
	var back = function() {
		...
		if (choice) {
			location.href = '/noticeViews/boardList.do';
		} else {
			alert('취소되었습니다.');
		}
	}
...
<body>
	...
	<form name="write"
		action=<c:if test="${boardInfo.get('key') eq 1}">"<c:url value="/noticeViews/write.do" />"</c:if>
			...
			<div id="writeMain">
				<table id="tbMain">
					...
					<tr>
						...
						<td><input type="hidden" name="con_no"
							value="${boardInfo.get('con_no')}" /> <input type="text"
							name="con_title" class="txtBox" size="50" maxlength="40"
							placeholder="제목을 입력하세요." value="${boardInfo.get('con_title')}">
						</td>
					</tr>
					<tr>
						<td style="background-color: #EDEDED;">내용</td>
						<td><textarea name="con_txt" class="txtBox" id="txtContents"
								cols="50" rows="20" maxlength="1000" placeholder="내용을 입력하세요.">${boardInfo.get('con_txt')}</textarea>
						</td>
					</tr>
				</table>
				<c:if test="${boardInfo.get('key') eq 1}">
					<table>
						...
						<tr>
							...
							<td><input type="password" class="txtBox"
								name="con_password" maxlength="10" placeholder="＊＊＊＊"></td>
						</tr>
					</table>
				</c:if>
			</div>
